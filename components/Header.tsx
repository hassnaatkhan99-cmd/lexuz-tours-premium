  ...
  return (
    <header className="sticky top-0 z-50 px-3 py-3">
      <div className={`mx-auto flex w-full max-w-[1280px] items-center justify-between gap-4 rounded-[26px] border border-white/65 bg-white/74 px-4 shadow-[0_26px_86px_rgba(7,31,8,.17)] backdrop-blur-[30px] transition-all duration-500 supports-[backdrop-filter]:bg-white/66 sm:px-6 xl:px-8 ${scrolled ? "h-[62px] rounded-[22px] shadow-[0_20px_64px_rgba(7,31,8,.18)]" : "h-[74px]"}`}>
        <Link href="/" className="focus-ring group flex min-w-0 shrink-0 items-center rounded-2xl p-1 transition hover:bg-white/55" aria-label="Lexuz Tours home">
          <Image src="/icons/android-chrome-192x192.png" alt="Lexuz Tours & Adventures" width={192} height={192} unoptimized className={`w-auto object-contain transition-all duration-300 group-hover:scale-[1.03] sm:hidden ${scrolled ? "h-10" : "h-12"}`} priority />
          <Image src="/brand/lexuz-logo-header.webp" alt="Lexuz Tours & Adventures" width={1200} height={500} sizes="(min-width: 1024px) 192px, 154px" className={`-my-3 hidden w-auto object-contain transition-all duration-300 group-hover:scale-[1.02] sm:block ${scrolled ? "h-16" : "h-20"}`} priority />
        </Link>
