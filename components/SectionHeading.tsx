export function SectionHeading({ eyebrow, title, copy, center = false, level = "h2" }: { eyebrow?: string; title: React.ReactNode; copy?: string; center?: boolean; level?: "h1" | "h2" }) {
  const Heading = level;
  return (
    <div className={`reveal-on-scroll mb-8 ${center ? "mx-auto max-w-3xl text-center" : "max-w-3xl"}`}>
      ...
      <Heading className="text-3xl font-black tracking-tight text-lexuzNeutral-100 md:text-5xl">{title}</Heading>
