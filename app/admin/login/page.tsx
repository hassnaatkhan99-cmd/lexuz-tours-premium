  ...
  return (
    <section className="container-page grid min-h-[70vh] place-items-center py-14">
      <div className="w-full max-w-md rounded-lg border border-forest-900/10 bg-white p-7 shadow-premium">
        <SectionHeading eyebrow="Admin" title="Login" copy="Enter the admin password to review bookings and payments." level="h1" />
        {!configured ? (
        ...
        <form action="/api/admin/login" method="post" className="grid gap-4">
          <label className="grid gap-2 text-sm font-bold">
            Admin Password
            <input name="password" type="password" required autoComplete="current-password" className="focus-ring rounded-md border border-neutral-200 px-4 py-3 font-normal" placeholder="Enter password" />
          </label>
          <button className="focus-ring rounded-md bg-forest-800 px-5 py-3 font-black text-white">Login</button>
        </form>
      </div>
    </section>
  );
